...
static void getPrivateKeyFromSubSeed(const uint8_t* seed, uint8_t* privateKey)
{
    KangarooTwelve(seed, 32, privateKey, 32);
}

static void getPublicKeyFromPrivateKey(const uint8_t* privateKey, uint8_t* publicKey)
{
    point_t P;
...
static void getIdentityFromPublicKey(const uint8_t* pubkey, char* dstIdentity, bool isLowerCase)
{
...
static std::string getIdentity(const uint8_t* pubkey, bool isLowerCase)
{
    uint8_t publicKey[32] ;
    memcpy(publicKey, pubkey, 32);
    char identity[64] = {0};
...
static void getTxHashFromDigest(const uint8_t* digest, char* txHash)
{
    bool isLowerCase = true;
    getIdentityFromPublicKey(digest, txHash, isLowerCase);
}

static void getPublicKeyFromIdentity(const char* identity, uint8_t* publicKey)
{
...
static bool checkSumIdentity(const char* identity)
{
    unsigned char publicKeyBuffer[32];
    for (int i = 0; i < 4; i++)
    {
